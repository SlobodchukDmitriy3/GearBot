...
class MyClient(discord.Client):
    async def Klubnika(self):
        ...
        async def on_raw_reaction_add(self,payload):
            ...
            message = await channel.fetch_massage(payload.message_id)
            member = utils.get(message.guild.members,id=payload.user_id)
        try:
          emoji = str(payload.emoji)
          role = utils.get(message.gild.roles, id = config.ROLES[emogi])
          if(len([i for i in member.roles if i.id not in config.EXCLORES]) <= config.MAX.ROLES.PER.USER):
                await member.add_roles(role)
                print('SUCCES User {0.display_name} has been granted with role {1.name}'.format(member,role))
          else:
            await message.remove_reaction(payload.emoji, member)
        except KeyError as e:
            print('[ERROR] To many roles for user' + emoji)
        except Exception as e:
            print(repr(e))
